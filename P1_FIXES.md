...
- **第二轮修复（现行）**：坐标改为**冻结的、版本化的参考表**
  ...
  `scripts/build_reference_coordinates.R` 从 **2014–2017 冻结训练期**一次性
...
feedback2.0 对应的主要工程任务已经完成：

1. Ridge 与 area-Poisson 使用相同 lambda 网格和嵌套时序调参；所有候选值必须
   在相同内层折收敛，否则不能参与选择。
2. horizon=1 与 horizon=12 分开滚动回测；2025 仅作回顾性描述。
3. 训练期 log-normal/Duan 校正在六个年度折上完成；平均 RMSE 均差于未校正
   Ridge，因此未采用全局乘数校正，也不再作 Jensen/漂移因果分解。
4. 下载 ID 集合、转换哈希、发生/报案日期、模型工件、CI、renv 和边界测试已补齐。

仍属研究拓展而非发布阻塞项：context 多折消融、带正则的 NB/ZINB、预测区间和
外部协变量。
